					      ...
					      int port)
{
	...
	mdio_ctrl = sja1105_pcs_read(priv, port, MDIO_MMD_VEND2, MDIO_CTRL1);
	if (mdio_ctrl < 0)
		return mdio_ctrl;

	if (mdio_ctrl & MDIO_AN_CTRL1_ENABLE) {
		rc = sja1105_pcs_write(priv, port, MDIO_MMD_VEND2, MDIO_CTRL1,
				       mdio_ctrl & ~MDIO_AN_CTRL1_ENABLE);
		if (rc < 0)
			return rc;
	}
	...
	rc = sja1105_pcs_write(priv, port, MDIO_MMD_VEND2, DW_VR_MII_DIG_CTRL1, rc);
	if (rc < 0)
		return rc;
