...
#define ICC_CORE_MEM_SPACE (CFG_BAREMETAL_SYS_SDRAM_SHARE_SIZE / \
		CONFIG_MAX_CPUS) /* share memory size for each core icc */
#define ICC_CORE_MEM_BASE(x) (CFG_BAREMETAL_SYS_SDRAM_SHARE_BASE + \
...
#define ICC_TRG_CORE (*(volatile unsigned *) \
		...
		+ CFG_BAREMETAL_SYS_SDRAM_SHARE_SIZE - 8))
#endif
